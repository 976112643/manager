...
class TransferController extends IndexController
{
    ...
    /** 开始状态*/
    protected $type_begin = 30;
    /** 结束状态*/
    protected $type_end = 39;
    ...
    public function index()
    {
        ...
        if (strlen(trim(I('keywords')))) {
            $map['m_1.mobile|m_2.mobile|order_no|deal_no'] = array(
                'like',
                '%' . I('keywords') . '%'
            );
        }
        /* 转出账户 */
        if (strlen(trim(I('from_member')))) {
            $map['m_1.mobile'] = array(
                'like',
                '%' . trim(I('from_member')) . '%'
            );
        }
        /* 转入账户 */
        if (strlen(trim(I('to_member')))) {
            $map['m_2.mobile'] = array(
                'like',
                '%' . trim(I('to_member')) . '%'
            );
        }
