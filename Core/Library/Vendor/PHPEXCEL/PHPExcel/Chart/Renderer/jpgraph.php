...
class PHPExcel_Chart_Renderer_jpgraph
{
 ...
 // function _formatPointMarker()
    ...
    {
        $datasetLabelFormatCode = $this->_chart->getPlotArea()
 ...
 // function _formatDataSetLabels()
    private function _percentageSumCalculation($groupID, $seriesCount)
    {
        ...
        for ($i = 0; $i < $seriesCount; ++ $i) {
            if ($i == 0) {
                $sumValues = $this->_chart->getPlotArea()
                    ->getPlotGroupByIndex($groupID)
                    ->getPlotValuesByIndex($i)
                    ->getDataValues();
            } else {
                $nextValues = $this->_chart->getPlotArea()
                    ->getPlotGroupByIndex($groupID)
                    ->getPlotValuesByIndex($i)
                    ->getDataValues();
 ...
 // function _renderLegend()
    private function _renderCartesianPlotArea($type = 'textlin')
    {
        ...
        $rotation = $this->_chart->getPlotArea()
 ...
 // function _renderRadarPlotArea()
    ...
    {
        $grouping = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotGrouping();
        ...
        $seriesCount = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotSeriesCount();
        $seriesPlots = array();
        ...
        for ($i = 0; $i < $seriesCount; ++ $i) {
            $dataValues = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotValuesByIndex($i)
                ->getDataValues();
            $marker = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotValuesByIndex($i)
                ->getPointMarker();
            ...
            $dataLabel = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
 ...
 // function _renderPlotBar()
    private function _renderPlotScatter($groupID, $bubble)
    {
        $grouping = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotGrouping();
        $scatterStyle = $bubbleSize = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotStyle();
        ...
        // Loop through each data series in turn
        for ($i = 0; $i < $seriesCount; ++ $i) {
            $dataValuesY = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotCategoryByIndex($i)
                ->getDataValues();
            $dataValuesX = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotValuesByIndex($i)
                ->getDataValues();
 ...
 // function _renderPlotScatter()
    ...
    {
        $radarStyle = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotStyle();
 ...
 // function _renderPlotRadar()
    ...
    {
        $contourStyle = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotStyle();
 ...
 // function _renderPlotContour()
    private function _renderPlotStock($groupID)
    {
        $seriesCount = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotSeriesCount();
        $plotOrder = $this->_chart->getPlotArea()
            ->getPlotGroupByIndex($groupID)
            ->getPlotOrder();
    ...
    private function _renderPieChart($groupCount, $dimensions = '2d', $doughnut = False, $multiplePlots = False)
    {
        ...
        for ($groupID = 0; $groupID < $iLimit; ++ $groupID) {
            $grouping = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotGrouping();
            $exploded = $this->_chart->getPlotArea()
                ->getPlotGroupByIndex($groupID)
                ->getPlotStyle();
            if ($groupID == 0) {
                $labelCount = count($this->_chart->getPlotArea()
 ...
 // function _renderContourChart()
    private function _renderCombinationChart($groupCount, $dimensions, $outputDestination)
    {
        ...
        for ($i = 0; $i < $groupCount; ++ $i) {
            $dimensions = null;
            $chartType = $this->_chart->getPlotArea()
 ...
 // function _renderCombinationChart()
    public function render($outputDestination)
    {
        ...
        if ($groupCount == 1) {
            ...
            $chartTypes = array();
            for ($i = 0; $i < $groupCount; ++ $i) {
                $chartTypes[] = $this->_chart->getPlotArea()
                    ->getPlotGroupByIndex($i)
                    ->getPlotType();
            }
